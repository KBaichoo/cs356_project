-- This only does analysis on packages with single binary + c++11 or newer
select creation_year, avg(smart_ptr_detected + named_cast_detected + fortified_source + stack_protector + PIE + ro_relocation + immediate_binding) as points from  (
select substr(creation_date, 1, 4) as creation_year, package_name,
CASE
    WHEN (
...
) group by creation_year order by creation_year;
