...
BINARY_PATH_FINDER_CMD = "%s" % os.path.join(
    os.getcwd(), 'src/binary_finder.sh')
DATE_FMT = '%Y-%m-%d'


class DetectionHarness:
    def __init__(self, package_infos, start_offset, count, github_repo_whitelist, features, skip_bisect):
    ...
    def _get_binary_paths(extraction_path):
        ...
        try:
            # Get the binary path.
            binary_paths = subprocess.check_output(
                BINARY_PATH_FINDER_CMD.split()).splitlines()
        except Exception as e:
            raise e
        finally:
            # Return to earlier working directory.
    ...
    def _run_git_bisection(git_repo_url, features):
        try:
            ...
            data['creation_timestamp'] = first_commit.authored_datetime.strftime(
                DATE_FMT)
            ...
            for feature in features:
                ...
                introduction_hash = DetectionHarness._run_git_bisect(
                    head_commit, first_commit, feature)
        ...
        for package_info in self._package_infos[self._start_offset:]:
            ...
            try:
                ...
                if (not self._skip_bisect and git_repo_url and
                    ...
                    git_bisection_data = self._run_git_bisection(
                        git_repo_url, self._features)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--start-offset', help='package index to start at', default=0)
